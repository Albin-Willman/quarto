...
Piece.prototype = {
  ...
  save: function() {
    ...
    doSaveObject(Pieces, doc, this);
  }
};

doSaveObject = function(obj, doc, that){
  if(that.id){
    obj.update({ _id: that.id}, { $set: doc }, function(error, result) {
      that._id = result;
    });
  } else {
    obj.insert(doc, function(error, result) {
      that._id = result;
    });
  }
}
